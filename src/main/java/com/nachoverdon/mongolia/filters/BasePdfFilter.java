...
/**
 * This class uses WkHtmlToPdf to convert a page rendered with Magnolia into a PDF. To use it,
 * extend from it and override its methods. Then add the path to your extended filter on your
 * Magnolia configuration on:
 * <pre>{@code
 *  config:
 *    server:
 *      filters:
 *        cms:
 *          # Make sure to put your filter BEFORE the "rendering" filter or it wont work)
 *          yourPdfFilter:
 ...
 * {@link #checkCondition}
 *      Must be overridden. Checks whether it should proceed with the PDF conversion. Override this
 *      class to check if the current page is should be converted, for example, by checking the node
 *      page's template.
 *
 * {@link #getParameters}
 *      A List of wkhtmltopdf parameters. Automatically includes ["-", "-"] as last parameters,
 *      so you don't need to include them.
 *      <a href="https://wkhtmltopdf.org/usage/wkhtmltopdf.txt">wkthmltopdf docs</a>
 *
 * {@link #shouldDownload}
 *      Whether the file should be served or you want to do something else with it, like store it as
 *      a Resource Node.
 *
 * {@link #getFileName}
 ...
 * {@link #download}
 *      Will be triggered when {@link #shouldDownload} returns true. By default it serves the PDF
 *      file.
 *
 * {@link #action}
 ...
 *
 */
public abstract class BasePdfFilter extends BaseFilter {
  ...
  protected void download(InputStream inputStream) throws IOException {
    ...
    filterParameters.getResponse().setHeader("Content-Disposition", "attachment; filename="
        + fileName + ".pdf");
  }
